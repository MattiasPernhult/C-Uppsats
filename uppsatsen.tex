...
\uek{Det finns två principiella sätt att strukturera relaterade artiklar. Aningen per papper (som ni gör här) eller per ämne som är relevant. Oavsett så tycker jag att ni borde skriva om hur ni har fått fram dem (sökning i vilka databaser och vilka söktermer som ni ansett vara relevanta).}
...
\begin{enumerate}
\item
\ugh{Kan vårt bedömningssystem minska tidsåtgången för lärares bedömningsprocess av programmeringsuppgifter?} \uek{Ni bör nog formulera det som ett bedömningssystem av den här typen. Annars är frågan lite väl begränsad som forskningsfråga, då ni per automatik har sagt att ni inte kan generalisera något svar }
\item
...
Eftersom studiens syfte är att konstruera, implementera och utvärdera en systemartefakt valdes en metodik som presenteras av Nunamaker m. fl. [Nunamaker] eftersom det är en erkänd och välbeprövad process för forskning inom informationssystem [källa]. 
\uek{jag måste rätta mig, för att få högsta betyg måste Ni  argumentera för/emot och jämföra med  metoder ni inte använder. Varför 'är Nunamaker lämoligare här än t.ex. design research?}
Författarna definierar en iterativ femstegsprocess för hur sådan forskning bör bedrivas vilken innefattar att: \textit{(1) konstruera ett konceptuellt ramverk} \textit{(2) utveckla en systemarkitektur} \textit{(3) konstruera och analysera systemet} \textit{(4) implementera systemet} och \textit{(5) utvärdera systemet}. För att ytterligare säkerställa studiens kvalité ämnar den att följa de sju riktlinjer för forskning inom informationssystem definierade av Hevner m.fl. [Hevner]. 
...
\ugh{Anledningen till att vi inte väljer en kvantitativ undersökningsmetod som exempelvis enkäter med fördefinierade svar beror på att de endast ger en ytlig förståelse i ämnet till skillnad från intervjuer [seminarieboken].}\uek{jag måste rätta mig, för att få högsta betyg måste Ni  argumentera för/emot och jämföra med  metoder ni inte använder.} Vid intervjuer finns det även utrymme för att ställa följdfrågor till respondenten samt att det går att förtydliga frågorna vid eventuella missuppfattningar, något som inte är möjligt vid fördefinierade enkätundersökningar [seminarieboken]. Informationen som vi samlar in från litteraturstudien kommer att användas för att urskilja de befintliga systemen. Därigenom kan vi identifiera funktioner som vi anser saknas i dessa system vilket gör att vi, tillsammans med informationen vi erhåller från intervjuerna, kan definiera krav som systemets ska uppfylla.

\subsubsection{Utveckla en systemarkitektur}

Enligt Nunamaker m. fl. innefattar följande steg att bestämma systemets komponenter och definiera förhållandet mellan komponenterna samt att identifiera och definiera systemets funktionalitet []. \ugh{Vi kommer att använda den insamlade information från föregående steg för att definiera komponenterna och systemfunktionerna.} \uek{Dels bör ni inte beskriva det är i futurum, dels så bör ni ha med lite mer detlajer om hur ni gick tillväga. inkludera gärna en referns till var i rapporten läsaren kan hitta resultaten från det här steget.} Nunamaker m.fl. påpekar betydelsen av att definiera mätbar funktioner för systemet eftersom det underlättar vid systemutvärderingen []. \uek{Och var hittar jag den funktionen i er rapport?}

\subsubsection{Konstruera och analysera systemet}

I detta steg är syftet att vi ska att identifiera möjliga lösningar, tekniker och språk för att kunna implementera systemet i kommande steg []. Möjliga lösningar kommer att konstrueras vilka vi sedan analyserar innan vi slutligen väljer det bästa alternativet att gå vidare med och implementera. \uek{återigen, beskriv vilka aktiviteter \emph{ni} har gjort och var man hittar resultaten.}

\subsubsection{Implementera systemet}

Tanken med följande steg är att implementera en prototyp som ska utvärderas i kommande steg. Prototypen kommer att ge insikter angående konceptets genomförbarhet och dess möjlighet att besvara frågeställningen. \uek{återigen, beskriv vilka aktiviteter \emph{ni} har gjort och var man hittar resultaten.}

\subsubsection{Utvärdera systemet}

Hevner presenterar i sin tredje riktlinje \textit{Design Evaluation} olika metoder för utvärdering av informationssystem. Den metod som vi anser är lämpligast för vår studie är en experimentell utvärderingmetod, mer specifikt kontrollerade experiment, eftersom det innebär att studera en implementerad prototyp i en kontrollerad miljö. Vi kommer att utföra experiment på lärare och studenter vilka är de två huvudgrupperna i studien. Målet med experimenten för lärare är att besvara frågeställningen \textit{Kan vårt bedömningssystem minska tidsåtgången för lärarens bedömningsprocess av programmeringsuppgifter?}, medans experimenten för studenter har som mål att besvara \textit{Kan vårt bedömningssystem hjälpa studenterna att utveckla deras programmeringskunskaper?}.  \uek{återigen, beskriv vilka aktiviteter \emph{ni} har gjort och var man hittar resultaten.}

\del{Vi hade kunnat använda en analytisk utvärderingsmetod där artefakten studeras analytiskt eftersom det saknas en fungerande prototyp, men eftersom vi kommer ha en fungerande prototyp anser vi att denna utvärderingsmetod inte lämpar sig för vår studie.} \uek{Återigen: Argumentera inte emot saker som ni inte gör. Bättre att ta bort helt.}
...
\uek{Ni bropde besriva lite mer detlajerat hur ni utformat enkäten, t.ex. att ni använt en Likert-skala. Ni kanske har nytta av Burgess, T.F., 2001. A general introduction to the design of questionnaires for survey research. University of Leeds. Eller har vi redan diskuterat detta?}

Vid genomförandet av användartestet och den andra enkäten kommer vi ej närvara i rummet eller ta tid, detta för att undvika att skapa onödig press på studenten. Vi tycker även att det är viktigt att poängtera för studenten att all information som vi samlar in är helt anonym för att studentens åsikt ska vara sanningsenlig. \uek{Ni kanske ska diskutera validiteten i era emprisiskt insamlade resultat, både gällande intervjuer och de som provade systemet åt er. Läs gärna Easterbrook, S., Singer, J., Storey, M.-A., Damian, D., 2008. Selecting Empirical Methods for Software Engineering Research, in: Guide to Advanced Empirical Software Engineering. Springer, pp. 285?311, osm jag tror ni kan ha stor nytta av.
}
...
\subsection{Intervjuer}
\uek{Jag tycker att ni  har en bra beskrivning av de intervjuer ni har genomfört. Ni kanske ska ta med era intervjufrågor i ett appendix i rapporten också. I slutet av denna sektionen bör ni dock sammanfatta vad intervjuresultaten medför för den typ av system ni vill bygga, en kort kravspec i form av en lista eller tabell. Ni kan ju senare i diskussionskapitlet at upp alternativa sätt att lösa lärarnas problem (om ni nu kommit på några).}

\ugh{Vi genomförde tre intervjuer med Farid Naisan, Rolf Axelsson och Jonas Wahlfrid vilka alla är anställda som lärare vid Malmö Högskola.} \uek{Ni bör fundera på om ni verkligen ska skriva ut namne på lärarna. Bättre att kalla dem med nummer eller fejkade namn. Är de tillfrågade om att medverka med namn i studien?} Resultaten från intervjuerna presenteras enligt de ämnesområden som intervjufrågorna berörde.
...
Rolf uppskattade att bedömningen av uppgifter tar 180 minuter per student
...
\caption{Allmäna användarupplevelsen av prototypen \uek{Om ni använder en Likert-skaa så måste ni förklara vilka värdeord ni har förklarat de olika siffrorna med.}}
\label{fig:GeneralUX}
\end{figure}

Som diagrammet visar \uek{Det är ju ganska lätt att lägga in referenser till vilka diagram som avses i LaTeX. Gör det!} är åsikterna från testpersonerna lite olika speciellt när det kommer till detaljrikedomen i återkopplingen. Tre personer upplevde att återkopplingen tydligt visar vad felet i programmet är. Dock upplevde tre personer att återkopplingen som vår prototyp gav var svårtydlig och svårförstådd. De flesta av testpersonerna upplevde att vår prototyp var enkel att använda eftersom navigeringen var tydlig och att det var tydligt hur man skickade in en uppgift för bedömning. 
...
\caption{Använda vår prototyp vid programmeringskurser \uek{Ett cirkeldiagram dör alla svar är likadana säger ju inte så mycket. Fundera på om ni i så fall ska ta bort diagrammet och bara skriva det i texten?}}
...
\caption{Upplevelsen av direkt återkoppling \uek{Samma kommentar som ovan.}}
...
\uek{referenslsitan stämmer ju int emed texten, men det vet ni väl?}
